...
| `duckdb_threads.py` | O `threads` do DuckDB na ingestão das tabelas Delta que a migração gravou (o `--root` dela): por valor de `threads`, o padrão do DuckDB vezes 1 a 5, cada configuração num processo novo com o motor `DuckDBEngine` do pacote, a partição da primeira tabela `materializada` (`ingest(..., materialize=True)`) e `agregada` pela view (`SELECT count(*), max(COLUMNS(*))`), e as tabelas `em série` na sessão principal e em `sessões a mais`, uma por tabela, como `run.ingest`; o melhor de três de cada medida, o pico de memória do processo sobre a base e as linhas lidas contra as do log. O banco de cada configuração fica na pasta temporária do motor e sai no `cleanup`. | As leituras do delta-rs e do DuckDB sob a raiz (`ListBucket`, `GetObject`). | `output/duckdb_threads_*.txt` |
...
| Quantas `threads` do DuckDB a ingestão do S3 pede, e o padrão, um por núcleo, basta? | `duckdb_threads.py`, seção 3 e `DT-5` |
