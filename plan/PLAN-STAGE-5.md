...
class RedshiftEngine:
    ...
    def audit(self, table: sa.Table, partitions: list[str] | None, uri: str | None = None, version: int | None = None,
              foreign_keys: bool = False, key_scope: str | None = None,
              referenced: Mapping[str, tuple[str, int]] | None = None) -> object: ...   # AuditReport da etapa 4
...
- **`stream`** vai sempre por `UNLOAD` (decisão do usuário de 2026-09-23). O driver lê o resultado
  ...
  `select * from (<texto>) as t limit 0` no cursor (proposta, à espera do usuário); outra contagem sobe
...
| Resultado vazio | `test_stream_empty_result` | Uma conexão de mentira em que o `UNLOAD` não grava manifesto: com `pg_last_unload_count()` em 0, o `stream` sai sem lote e com o esquema do statement, e o texto com o do `row_desc`; com 2, a falta do manifesto sobe; no alvo (`redshift`), um `select` sem linha. |
...
- **[decisão] A exportação de uma partição com `Double` não finito.** O rodapé que o `UNLOAD`
  grava deixa o `NaN` fora do mínimo e do máximo, e o leitor Parquet do DuckDB perdeu a linha
  (leitura de 2026-09-23, [`POC.md`](POC.md)); em `register`, o arquivo fica com esse rodapé. A
  proposta é que `export_partition` exporte por `rewrite` a partição com `columns_without_min_max`
  não vazio, qualquer que seja o `mode`, porque só o `rewrite` grava o rodapé sem a estatística
  dessas colunas.
- **[decisão] O esquema do `stream` vazio de um texto.** O `UNLOAD` sem linha não grava arquivo, e
  o texto não tem o esquema do statement. A proposta é o `schema_from_row_description` de
  `select * from (<texto>) as t limit 0` pelo cursor, só quando o resultado sai vazio.

As decisões que o usuário tomou em 2026-09-23 estão escritas nas seções que as descrevem: `connect`
sem comando de conferência do `USE`, e a tabela de controle criada só pelo usuário
