...
Read before `serialize_db.schema` and `serialize_db.sql` (stages 1 and 2), a DDL rule per dialect or generated SQL text. Each fact ends with the `docs/` file that details it, and `tests/proof_of_concept/` holds the API details as assertions. A fact found in a session is appended here, under the heading it belongs to.
