...
def arrow_type(column: sa.Column) -> pa.DataType:
    ...
    for sa_type, arrow in ARROW_TYPES:
        if isinstance(column.type, sa_type):
            return arrow
    raise AssertionError(f"{column.table.name}.{column.name}: o tipo {column.type!r} não está em ARROW_TYPES")
...
def test_the_keys_transcribed_for_the_fixture_are_the_models() -> None:
    ...
    for name, table in Base.metadata.tables.items():
        ...
        assert sorted(keys) == sorted(source.UNIQUE_KEYS[name]), name
