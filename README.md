...
| `test_client_model.py` (em `tests/`) | O modelo cliente de `tests/client_model/`, a cópia corrigida do modelo de referência, contra o original: as tabelas e as colunas na mesma ordem com a coluna de partição no fim, `BigInteger` só nas chaves e `String(n)` só onde havia `String`, a nulidade, as chaves sem `DEFERRABLE` e sem `autoincrement`, os comentários e a partição declarada igual à da base. | Nenhum. |
...
permissões de S3 da suíte S3 sob a raiz, a não ser que `SERIALIZE_DB_REDSHIFT_IAM_ROLE` nomeie um
papel associado ao namespace. Fora das raízes informadas, o que uma sessão grava é `.pytest_cache/`
na raiz do repositório, do próprio pytest.
