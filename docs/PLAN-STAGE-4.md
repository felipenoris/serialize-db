...
| `export_partition(table, value)` | O `RecordBatchReader` da partição, passado por `cast`, para `publish_partition`, sem outro comando na conexão até o fim da escrita; ou `COPY ... TO '<uri>/<coluna de partição>=<valor>/<execution_id>.parquet' (FORMAT parquet, RETURN_STATS)` mais `register_files` para a tabela que não cabe na memória. |
...
materializadas e dimensões em view, um `select` com `join`, auditoria, exportação da partição) sobre um
...
reprova a chave repetida dentro da partição e a repetida
contra uma partição já publicada, e o órfão de chave estrangeira com a tabela referenciada fora do
