...
| `src/serialize_db/` | The package: `errors.py` (`ContractError`, `SqlError`), `schema.py` (stage 1: the Arrow and Delta schemas, `TableOptions`, `sql_type`, the DDL text with every identifier quoted (`temporary=True` emits `CREATE TEMP TABLE`, unused by the plan, user request of 2026-09-22), `cast` for batch, table and reader, `check_models`, the schema files in canonical JSON and SQL; the Delta schema drops the Arrow `PARQUET:field_id`, which made `delta_scan` read every column as null, 2026-09-21), `sql.py` (stage 2: `param` as `:nome`, the quoted prefixed copy of `prefixed`, `render` by the `duckdb-engine` and `sqlalchemy-redshift` dialects with `paramstyle="named"` and line ends stripped, `bind` with quoted regions intact, `referenced_tables`, the SQL text files with the `{prefix}` sentinel that only `read_sql` fills; `SENTINEL` protected for the engines) and `cli.py` (`serialize-db schema write|check --metadata modulo:atributo <folder>` and `serialize-db sql write|check --metadata ... --statements modulo:atributo <folder>`, public only in `main`); `pyproject.toml` pins the runtime dependencies (`deltalake`, `pyarrow`, `sqlalchemy`, `duckdb`, `duckdb-engine`, `sqlalchemy-redshift`) and the `docs` group (`pdoc`). |
