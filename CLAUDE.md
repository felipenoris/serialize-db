...
- DuckDB has `http_proxy`, `http_proxy_username` and `http_proxy_password` and nothing like
  ...
  the proxy as `Proxy-Authorization: Basic`. `prepare_offline.sh` does the split; a DuckDB HTTP
  call anywhere else in the project would hit the same parse error. `docs/POC.md`
