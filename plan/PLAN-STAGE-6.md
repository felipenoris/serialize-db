...
| `run.publish_redshift(*tables, max_workers=1)` | Entra com a [etapa 8](PLAN-STAGE-8.md), que a implementa: a publicação, uma conexão por tabela em paralelo, limitada pelas slots do WLM; dois `COPY` e dois `UNLOAD` em conexões distintas passaram em paralelo no ambiente alvo em 2026-09-21 ([`POC.md`](POC.md)). |
...
| `serialize-db run` | `--root`, `--environment`, `--engine`, `--partition`, `--execution-id`, `--metadata modulo:atributo` (o `MetaData` dos modelos, como em `schema` e `sql`) e `modulo:funcao` do pipeline, que recebe `run`; código de saída 0, 1 na reprovação da auditoria, 2 no conflito. |
...
- **`publish_redshift`** entra com a [etapa 8](PLAN-STAGE-8.md), uma conexão por tabela no pool.
