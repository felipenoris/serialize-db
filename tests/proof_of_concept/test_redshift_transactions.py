"""Transações simultâneas no Redshift: duas publicações da etapa 8 que se cruzam no esquema do datashare.
...
seção "Transações concorrentes"): ``DELETE`` e ``UPDATE`` tomam o lock da tabela e o segundo espera o
primeiro terminar; sob isolamento de snapshot, dois escritores de linhas distintas confirmam, e sob o
serializável o segundo recebe ``1023``; o snapshot de uma transação nasce no primeiro ``SELECT``, DML
ou DDL dela; o ``LOCK`` no início força a ordem, mas não está na lista de comandos que a escrita por
datashare aceita. O banco do datashare informou isolamento ``UNKNOWN`` no ambiente alvo, e a escrita
por datashare exige snapshot no banco do produtor. Estes testes medem o que o esquema do datashare faz.

Cada cenário abre duas conexões próprias, A e B, pela resolução de ``connect_redshift``, e segue a
sequência da etapa 8 com ``INSERT ... VALUES`` no lugar do ``COPY``, que toma o mesmo lock. A roda até
o ponto de conflito e segura a transação aberta; B roda numa thread, e o relatório registra se B
...
``LOCK`` da tabela de controle no início da transação; e a linha de controle gravada por um ``UPDATE``
condicionado à versão lida, como primeiro comando. As tabelas são ``serialize_db_poc_<id>_*`` no
esquema de ``SERIALIZE_DB_TEST_REDSHIFT_SCHEMA``, apagadas no fim da sessão pela fixture
``redshift_session``; nada vai ao S3.
"""
...
@dataclass
class Step:
    """Um comando de um participante e o seu desfecho: o tempo, o erro do servidor, as linhas afetadas e o resultado."""
    ...
    def describe(self) -> str:
        """O desfecho numa linha: ``rótulo: 0.4 s, linhas afetadas: 2`` ou ``rótulo: erro em 12.1 s: ...``."""
...
class Participant:
    ...
    def __init__(self, name: str) -> None:
        ...
        # O SELECT paga o primeiro comando da sessão (10,8 s no ambiente alvo, 2026-09-21) antes do
        # cenário, e o pid é o que pg_terminate_backend recebe se a thread ficar presa.
        timeout = self.execute(Step("statement_timeout", f"SET statement_timeout TO {STATEMENT_TIMEOUT_MS}"))
    ...
    def start(self, steps: list[Step]) -> threading.Thread:
        """Roda os comandos numa thread, para o cenário ver se eles esperam pela transação do outro."""
...
class Tables:
    """A tabela de controle e a tabela de dados de cada ambiente num cenário, com os nomes como a sessão os cita."""
...
def create_tables(session: RedshiftSession, scenario: str) -> Tables:
    """A tabela de controle de ``serialize_db_publications`` com prod e dev na versão 1, e uma tabela de dados por ambiente com a partição publicada."""
    control = session.qualified(session.table(f"{scenario}_controle"))
    session.execute(f"CREATE TABLE {control} (table_name VARCHAR(127) NOT NULL, delta_version BIGINT NOT NULL, execution_id VARCHAR(127) NOT NULL, published_at TIMESTAMP NOT NULL)")
    session.execute(f"INSERT INTO {control} VALUES ('prod_x', 1, 'exec-0', getdate()), ('dev_x', 1, 'exec-0', getdate())")

    names = []
    for environment in ("prod", "dev"):
        name = session.qualified(session.table(f"{scenario}_{environment}_x"))
        session.execute(f"CREATE TABLE {name} (id BIGINT NOT NULL, execution_id VARCHAR(127) NOT NULL, data_str VARCHAR(10) NOT NULL)")
        session.execute(f"INSERT INTO {name} VALUES (1, 'exec-0', '{PARTITION}'), (2, 'exec-0', '{PARTITION}')")
        names.append(name)
...
def replace_partition(table: str, execution_id: str) -> list[Step]:
    """A troca da partição da etapa 8, com ``INSERT`` no lugar do ``COPY`` na staging: o mesmo lock de escrita."""
    return [
        Step("apaga a partição", f"DELETE FROM {table} WHERE data_str = '{PARTITION}'"),
        Step("insere a partição", f"INSERT INTO {table} VALUES (1, '{execution_id}', '{PARTITION}'), (2, '{execution_id}', '{PARTITION}')"),
    ]
...
def write_control_row(control: str, table_name: str, version: int, execution_id: str) -> list[Step]:
    ...
    return [
        Step("apaga a linha de controle", f"DELETE FROM {control} WHERE table_name = '{table_name}'"),
        Step("insere a linha de controle", f"INSERT INTO {control} VALUES ('{table_name}', {version}, '{execution_id}', getdate())"),
    ]


def waits(thread: threading.Thread) -> bool:
    """Verdadeiro quando a thread ainda roda depois de ``BLOCKED_AFTER`` segundos: o comando dela espera pela outra transação."""
    thread.join(BLOCKED_AFTER)
    return thread.is_alive()


def finish(thread: threading.Thread, participant: Participant, session: RedshiftSession, prefix: str) -> None:
    """Espera a thread do participante; presa depois do prazo, a sessão dela é encerrada, e o fato vai ao relatório."""
    thread.join(FINISH_WITHIN)
    if not thread.is_alive():
        return
    record(f"{prefix}.presa", f"{participant.name} em '{participant.current()}' depois de {FINISH_WITHIN:.0f} s")
...
def partition_origin(session: RedshiftSession, table: str) -> list[str]:
    ...
    rows = session.execute(f"SELECT DISTINCT execution_id FROM {table} WHERE data_str = '{PARTITION}'")
    return sorted(row[0] for row in rows)


def test_isolation_level_readings(redshift_session: RedshiftSession) -> None:
    """O nível de isolamento que o Redshift informa para os bancos que a sessão vê: leitura, nunca asserção."""
    session = redshift_session
    rows = session.execute("SELECT database_name, database_type, database_isolation_level FROM svv_redshift_databases ORDER BY database_name")
    record("redshift.transactions.isolation.svv_redshift_databases", rows)

    # A visão STV pode ser negada a um usuário comum, como stv_slices (42501): a negação também é leitura.
    try:
        record("redshift.transactions.isolation.stv_db_isolation_level", session.execute("SELECT * FROM stv_db_isolation_level"))
    except Exception as error:  # noqa: BLE001 - a negação é a leitura
        record("redshift.transactions.isolation.stv_db_isolation_level", f"{type(error).__name__}: {error}")


def test_writes_to_distinct_tables(redshift_session: RedshiftSession) -> None:
    """A e B escrevem, cada um numa transação, em tabelas distintas: a ingestão em paralelo e ``publish_redshift`` com uma conexão por tabela."""
    session = redshift_session
    tables = create_tables(session, "distintas")
    prefix = "redshift.transactions.distintas"
    a, b = Participant("A"), Participant("B")
    try:
        a.run([begin(), *replace_partition(tables.prod, "exec-a")])
        b.run([begin()])
        thread = b.start([*replace_partition(tables.dev, "exec-b"), commit()])
        record(f"{prefix}.b_espera_a", f"{waits(thread)} ({b.current()})")
        a.run([commit()])
...
def test_two_environments_write_distinct_control_rows(redshift_session: RedshiftSession) -> None:
    """prod e dev publicam ao mesmo tempo: tabelas de dados distintas e linhas distintas da tabela de controle.
    ...
    a, b = Participant("A"), Participant("B")
    try:
        a.run([begin(), *replace_partition(tables.prod, "exec-a"), *write_control_row(tables.control, "prod_x", 2, "exec-a")])
        b.run([begin(), *replace_partition(tables.dev, "exec-b")])
        thread = b.start([*write_control_row(tables.control, "dev_x", 2, "exec-b"), commit()])
        record(f"{prefix}.b_espera_a", f"{waits(thread)} ({b.current()})")
        a.run([commit()])
        finish(thread, b, session, prefix)
    finally:
    ...
    # O estado final bate com os desfechos: cada ambiente tem a partição e a linha de quem confirmou.
...
def test_two_publications_of_the_same_table(redshift_session: RedshiftSession) -> None:
    ...
    def publication(execution_id: str, version: int) -> list[Step]:
        return [
            Step("cria a staging", f"CREATE TABLE {staging} (id BIGINT, execution_id VARCHAR(127))"),
    ...
    a, b = Participant("A"), Participant("B")
    try:
        a.run([begin(), *publication("exec-a", 2)])
        b.run([begin()])
        thread = b.start([*publication("exec-b", 3), commit()])
        record(f"{prefix}.b_espera_a", f"{waits(thread)} ({b.current()})")
        a.run([commit()])
        finish(thread, b, session, prefix)
    finally:
...
def test_lock_on_the_control_table(redshift_session: RedshiftSession) -> None:
    """O ``LOCK`` da tabela de controle no início das duas transações, a forma documentada de forçar a ordem.

    O ``LOCK`` não está entre os comandos que a escrita por datashare aceita, e a recusa também é a
    leitura. Aceito, B espera no ``LOCK`` até o ``COMMIT`` de A, e o snapshot de B nasce depois dele:
    a versão que B lê dentro da transação é a que A gravou.
    """
    session = redshift_session
    ...
    try:
        ...
        if a.failed:
            ...
            return
        a.run([*replace_partition(tables.prod, "exec-a"), *write_control_row(tables.control, "prod_x", 2, "exec-a")])
        b.run([begin()])
        thread = b.start(
            [
                Step("LOCK", f"LOCK {tables.control}"),
                Step("lê a versão publicada", f"SELECT delta_version FROM {tables.control} WHERE table_name = 'prod_x'"),
...
def test_conditional_update_of_the_control_row(redshift_session: RedshiftSession) -> None:
    """A linha de controle gravada primeiro, por um ``UPDATE`` condicionado à versão lida antes da transação.

    As duas publicações leram a versão 1. O ``UPDATE`` de B espera o lock que o de A tomou; depois do
    ``COMMIT`` de A, o snapshot de B nasce no próprio ``UPDATE``, e o relatório diz se B atualizou 0
    linhas (viu a versão 2) ou recebeu um erro de serialização. Em qualquer dos dois, a publicação de
    B saberia que outra publicou antes e desfaria a transação.
    """
    session = redshift_session
    ...
    def update_if_unchanged(execution_id: str, version: int) -> Step:
        return Step(
            ...
            f"UPDATE {tables.control} SET delta_version = {version}, execution_id = '{execution_id}', published_at = getdate() "
            "WHERE table_name = 'prod_x' AND delta_version = 1",
        )

    a, b = Participant("A"), Participant("B")
    try:
        a.run([begin(), update_if_unchanged("exec-a", 2)])
        b.run([begin()])
        thread = b.start([update_if_unchanged("exec-b", 3)])
        record(f"{prefix}.b_espera_a", f"{waits(thread)} ({b.current()})")
        a.run([commit()])
        finish(thread, b, session, prefix)
    finally:
