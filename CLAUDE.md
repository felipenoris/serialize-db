...
| File | Subject |
| --- | --- |
| `README.md` | `uv sync --group dev`, the `pdoc` build into a folder of the user's choice, and only the commands: the package tests (the GitHub workflow's command, `tests/` without `tests/proof_of_concept/` and `tests/test_probes.py`), the AWS tests with and without Redshift (the variable `SERIALIZE_DB_TEST_REDSHIFT_SCHEMA` switches the suite on, `-m "not redshift"` off), the variables table, the probe commands, the migration script's commands; the file opens with the two workflows' badges, the tests one linking to its workflow page and the docs one to the published site; what each test does lives in the header of its file, and the suites' writes, permissions and report in the header of `tests/conftest.py` (user decision of 2026-09-21). Also the delta-rs credentials and proxy note, and why the folder is prepared for the target without internet, with the procedure in the `prepare_offline.sh` header. |
| `docs/` | The package documentation for `pdoc` (user decision of 2026-09-21): `docs/index.md` is the main page, included by the docstring of `src/serialize_db/__init__.py` (`.. include:: ../../docs/index.md`), with how the package works, the tutorial and the type mapping table; `uv run pdoc serialize_db --docformat restructuredtext -o site` builds it, `site/` is ignored. Every docstring example opens with `.. code-block:: python`, the command-line one with `.. code-block:: shell`: pdoc highlights the code of that directive only, and an indented block comes out plain (2026-09-22). The plan and the study documents are in `plan/`. |
...
| `src/serialize_db/` | The package: `errors.py` (`ContractError`, `SqlError`), `_files.py` (private: `write_files` and `diff_files`, the generated files that `schema` and `sql` write and diff, 2026-09-22), `schema.py` (stage 1: the Arrow and Delta schemas, `TableOptions`, `sql_type`, the DDL text with every identifier quoted (`temporary=True` emits `CREATE TEMP TABLE`, unused by the plan, user request of 2026-09-22), `cast` for batch, table and reader (text measured after the conversion to `string`, integer into `Numeric` through `decimal128(38, s)`, 2026-09-22), `check_models` (a foreign key must target the referred table's primary key or a `UniqueConstraint`, in the same column order, since 2026-09-22), the schema files in canonical JSON and SQL; the Delta schema drops the Arrow `PARQUET:field_id`, which made `delta_scan` read every column as null, 2026-09-21), `sql.py` (stage 2: each valueless `bindparam` written as `:nome` by `render`, so one statement serves a client's `sqlalchemy.Connection`, the engines and the files (user decision of 2026-09-22; `param` left with it), the quoted prefixed copy of `prefixed`, `render` by the `duckdb-engine` and `sqlalchemy-redshift` dialects with `paramstyle="named"` and line ends stripped, `bind` with quoted regions intact, `referenced_tables`, the SQL text files with the `{prefix}` sentinel that only `read_sql` fills; `SENTINEL` protected for the engines) and `cli.py` (`serialize-db schema write|check --metadata modulo:atributo <folder>` and `serialize-db sql write|check --metadata ... --statements modulo:atributo <folder>`, public only in `main`); `pyproject.toml` pins the runtime dependencies (`deltalake`, `pyarrow`, `sqlalchemy`, `duckdb`, `duckdb-engine`, `sqlalchemy-redshift`) and the `docs` group (`pdoc`). |
...
| `tests/client_model/` | The client model (user decision of 2026-09-21): the data model the client code presents to use the library, the copy of `tests/reference_model/` corrected as `plan/PLAN-STAGE-1.md` lists (one `Base`, each table once, `BigInteger` keys with `autoincrement=False`, no `DEFERRABLE`, `String(n)` from the readings, the partition column last with `Table.info["serialize_db"]`, comments, `Double` kept, no index at all, the two composite unique indexes of the original being `UniqueConstraint` because the composite foreign keys reference them and `create_all` on DuckDB refuses a unique index as target, user decision of 2026-09-22); `tests/test_client_model.py` diffs it against the original, creates the whole model on a `duckdb-engine` `Connection`, the sort key of each partitioned table decided by the user on 2026-09-21, and no decision of the model awaiting the user. The tests hand it to the package API as a client library would hand its own; the package holds no model. `statements.py` holds the fictitious pipeline's Core statements (`STATEMENTS`, what `serialize-db sql` takes by `--statements`) and `sql/` the eight SQL text files generated from them, with the `{prefix}` sentinel (stage 2, 2026-09-22). |
...
`plan/OPEN_QUESTIONS.md` before planning a session. Stage 1 closed on 2026-09-21: the client model in `tests/client_model/`, `serialize_db.schema` with `serialize_db.errors` and `serialize-db schema` in `serialize_db.cli`, `tests/test_schema.py` and the schema files in `tests/client_model/schema/` (the DDL generated from the type table without the dialect packages and `String` without length a `check_models` violation, both user decisions of 2026-09-21; every identifier double-quoted). The same day the plan moved to `plan/`, `docs/` became the `pdoc` site with `docs/index.md` as its main page, and `.github/workflows/` gained the package-tests workflow (passed on PR #46 after a first failure on the missing `v10` tag of `setup-uv`; `tests/test_probes.py` left it on the user's request) and the docs workflow, which failed three times on `main` on 2026-09-21 until the repository's Pages source became "GitHub Actions"; the manual run of 18:24 UTC published the site at <https://felipenoris.github.io/serialize-db/>. The user removed the corporate index from `pyproject.toml` the same day. `scripts/migrate_parquet_to_delta.py`, the stage 7 draft on `serialize_db.schema`, `deltalake` and DuckDB (`plan/PLAN-STAGE-7.md`, section "A migração adiantada"), was written and tested on the fixture on 2026-09-21 (PR #49), after the user decided the sort keys of the four partitioned tables; its report was the first `delta_scan` over a table created by `delta_schema` and read every column as null because of `parquet.field.id`, fixed in `delta_schema` the same day. The next step is the user running it in the target on the copy of the production base, one table at a time, with the two measurements that section lists (the script measures the `cad_lancamentos` partition itself with `--tables cad_lancamentos --partitions <valor>`, in both modes and with `--no-sort`); stages 3, 4 and 6 follow, and stage 7 absorbs the script. Stage 2's plan was reviewed on 2026-09-21 (`plan/PLAN-STAGE-2.md`): the draft was rewritten in the module's form (`compiled.binds` instead of `warnings.catch_warnings`, explicit loops, private names, `referenced_tables`), and the user kept `duckdb-engine` and `sqlalchemy-redshift` as `render`'s compilers (they enter the runtime dependencies with stage 2) and the `quote=True` prefixed copy, so the DML quotes every contract identifier like the DDL; the alternatives are measured in `plan/POC.md`; `sqlglot==30.18.0` entered the `dev` group on 2026-09-22 for the test that the Redshift text parses, and stage 2 has no decision awaiting the user. Stage 2 closed on 2026-09-22: `serialize_db.sql`, `SqlError`, `serialize-db sql write|check`, `tests/test_sql.py` (16 cases), `tests/client_model/statements.py` and the eight files of `tests/client_model/sql/`; `duckdb`, `duckdb-engine` and `sqlalchemy-redshift` became runtime dependencies and `redshift-connector` stayed in `dev`, because the dialect imports it only in `import_dbapi`; the run showed the versioned file with the sentinel parses with `sqlglot`, since the sentinel sits inside the quotes, so the test parses the versioned files, and `render` strips the space the compiler leaves before each line break (`plan/POC.md`); `probes/space.py` `SP-9` reads the runtime dependencies with the `dev` group since then. Stage 4's four decisions closed on 2026-09-22, before any code: the `loader` refuses with `SandboxError` a name already taken in the sandbox, `memory_limit` stays at DuckDB's default with the chosen value logged, the database is a file in a `tempfile.mkdtemp` folder that `cleanup` deletes, and the `AuditReport` carries up to 20 whole rows per failed check; the refusal brought `published(table)` into both engines' protocol and into `Execution`, and `SandboxError` into `serialize_db.errors` (`.claude/memory/decisions.md`). Stage 3's four decisions closed on 2026-09-22, before any code: `create_table` passes the table comment as the Delta `description` and `reconcile` syncs the description and the column comments (`set_table_description`, `set_column_metadata`), `storage_options` carries no credential at all, `register_files` registers min and max of the integer, date, `Double` and `String` columns, and `version_diff` refuses a cleaned log with `LogUnavailable` instead of falling back to the set difference; two probes of the same day back them, in `tests/proof_of_concept/test_deltalake.py` (`.claude/memory/decisions.md`, `.claude/memory/delta.md`). Stage 5's connection model closed on 2026-09-22 (user decision, after the temporary-table analysis of the same day, which the user reverted): the `exec_<id>_*` tables stay in the datashare schema, and the Redshift engine keeps one session per execution under a `threading.Lock`, with `stream` executing under it and its helper thread slicing `fetchmany`, no connection per thread; the DuckDB engine keeps its cursor per thread. The partition column was generalized the same day (user decision): any `String(n)` text column, `partition_source` optional and checked only when declared, the `AAAA-MM-DD` date being the current base's case; `check_models`, the docs, the migration script's folder pattern and stages 4, 6 and 7 follow it. The same day the user made the Core statement submitted to the engine the default path and the generated SQL text the optional migration path out of SQLAlchemy, with the requirement that the model and the statements stay acceptable to a `sqlalchemy.Connection` the client creates outside the library; the user accepted both proposals from that day's probes the same day: `render` writes a valueless `bindparam` as `:nome` (`param` left the module, the eight versioned files unchanged, the statement run on a `duckdb-engine` `Connection` in `test_sql.py`), and `UniqueConstraint` replaced the client model's two composite unique indexes, with `check_models` refusing a foreign key whose target is not the referred table's primary key or a `UniqueConstraint` in the same column order (DuckDB refuses both the unique index and a swapped order; the whole client model now creates on a `duckdb-engine` `Connection` in `test_client_model.py`); the migration ran successfully in the target, with its reports not yet available. The pending API decisions are listed per stage in `plan/OPEN_QUESTIONS.md`; stage 1's seven all closed on 2026-09-21 (`Text` as `VARCHAR(65535)` with `cast` measuring the 65,535-byte ceiling, `String(n)` measured in bytes, the comment optional in `check_models`, the `String(n)` lengths reviewed by the model's owner in the code, the Redshift distribution left `AUTO` with `svv_table_info` read after the first publication, the comments being a first draft the model's owner revises in the code, and the foreign key `cad_contratos` declared to `rel_contrato_operacao` removed, because the contract is in N operations and the target is not unique).
The review of 2026-09-22 (the user's request after the `PLAN.md` review commit) fixed three
`cast` defects (text in `large_string`, `string_view` and dictionary skipped the byte measure; a
cast without kernel escaped as `ArrowNotImplementedError`; the integer detour held only for
`Numeric(18, 2)`), moved the duplicated file helpers of `schema` and `sql` to `_files.py`, fixed
stale docstrings (`sql.py` framed the text as the default path, `SqlError` named `param`), fixed
test defects (an operator-precedence assertion, a shared-fixture cleanup outside `finally`, the
S3 cleanup count, the `BatchStream` sketch waiting forever after an error, thread exceptions
dropped by `run_in_threads`, untimed waits), and revised `plan/PLAN-STAGE-3.md` (table and
interface aligned, `open_table`, no `ExportMode` in stage 3, `read_back` with its own connection,
`publish_partition` returning its own commit's version, the migration script's `delta_stats`
absorbed). Proposals awaiting the user, in `plan/OPEN_QUESTIONS.md`: refusing tz-aware into naive
timestamps (stage 1), `expressions` in `rewrite` and `Storage` over `pyarrow.fs` (stage 3), the
single quote in partition values (stage 6), and whether pytest's temporary folder counts as a
write without authorization; the S3 and Redshift suite fixes wait for the target. The stale study
content of `tests/proof_of_concept/` (the old `param` and `render` of `test_sqlalchemy.py`, its
process-wide `@compiles` hook, decimal statistics and `version_diff` by set difference in
`test_deltalake.py`, month APIs in `test_stdlib.py`) is left for a separate session.
