...
local. ``test_s3_proof_of_concept.py`` e ``test_local_proof_of_concept.py`` herdam a classe, fornecem
as fixtures ``storage`` (a raiz da sessão), ``table_uri`` (a tabela ``operacoes`` gravada por
``write_sample_table``) e ``duckdb_connection`` (a conexão com as extensões daquele armazenamento) e
acrescentam os testes próprios do seu armazenamento. As medições vão para o relatório da sessão com
o prefixo do armazenamento, como ``local.timing.delta_scan.aggregate_first``.
"""
