...
| Primitiva | O que faz |
| --- | --- |
| `arrow_type(column)` | O `pa.DataType` da coluna pela tabela de tipos da documentação (`docs/index.md`): `Numeric(p, s)` em `decimal128(p, s)`, `DateTime` em `timestamp[us]` (`tz=UTC` com fuso), os demais por `isinstance` na ordem que põe `BigInteger` e `SmallInteger` antes de `Integer` e `Text` antes de `String`; `Float`, `LargeBinary`, `ARRAY` e `Interval` são `ContractError`, com tabela e coluna, e também o `Enum`, que deriva de `String` sem que nada confira a lista de valores, e o `Numeric` de precisão fora de 1 a 38, que o `decimal128` do PyArrow recusava com `ValueError`, ou de escala fora de 0 a `p`, que o delta-rs recusava com a `Exception` genérica e o DuckDB no DDL, ou acima de 37, a maior escala que a documentação do Redshift aceita (decisões do usuário de 2026-09-25). |
...
| `cast(data, table)` | A `pa.Table`, o `pa.RecordBatch` ou o `RecordBatchReader` lote a lote, convertido para `arrow_schema(table)` com `safe=True` e devolvido no mesmo tipo (`RecordBatch.cast` recusa o mesmo que `Table.cast`): as colunas do contrato presentes, na ordem do contrato; `large_string`, `string_view` e dicionário para `string`, o `arrow.uuid` que o PyArrow e o pandas inferem de um `uuid.UUID` no texto canônico de `str(valor)` (decisão do usuário de 2026-09-25), timestamps a microssegundos e UTC, inteiro em `Numeric(p, s)` por `decimal128(38, s)`, que recebe qualquer `int64`, antes de conferir `p`; recusa perda de precisão, `double` fora da escala, `timestamp` com hora numa coluna `Date` e `timestamp` com fuso numa coluna `DateTime` sem fuso e o inverso (as perdas que `safe=True` não acusa; o fuso, decisão do usuário de 2026-09-23), `struct` numa coluna JSON, texto acima de `String(n)` em bytes, texto numa coluna `Uuid` acima de 36 bytes (decisão do usuário de 2026-09-25), texto numa coluna `Text` e documento JSON acima de 65.535 bytes (o documento, decisão do usuário de 2026-09-23, [etapa 8](PLAN-STAGE-8.md)), medidos depois da conversão para `string`, um tipo sem conversão para o do contrato, nulo em coluna `NOT NULL` e um lote sem coluna alguma do contrato, com a tabela, a coluna e a instrução ao cliente na mensagem. Aceita `NaN` e infinito numa coluna `Double` (decisão do usuário de 2026-09-23): os dois escritores do Delta gravam o máximo sem o `NaN`, e o `delta_scan` responde a um filtro por intervalo conforme a poda, a questão aberta da [issue #59](https://github.com/felipenoris/serialize-db/issues/59). |
| `check_models(metadata)` | A lista de violações do contrato nos modelos, um texto por violação com tabela e coluna: tipo fora da tabela de tipos (inclusive o `Enum` e o `Numeric` de precisão fora de 1 a 38 ou de escala fora de 0 a `p`, até 37; decisões do usuário de 2026-09-25), `autoincrement` em chave inteira (o padrão `"auto"` inclusive), `Identity`, `String` sem comprimento, chave estrangeira `DEFERRABLE` ou cujas colunas apontadas não são a chave primária nem uma `UniqueConstraint` da tabela apontada, na mesma ordem (decisão do usuário de 2026-09-22: o DuckDB recusa o índice único como alvo e a ordem trocada, e o Redshift documenta a mesma exigência), `partition_by` com mais de uma coluna, sem a coluna ou com a coluna fora de `String(n)`, `partition_source` que a tabela não tem ou sem `partition_by`, tabela sem chave primária e sem `keys`. Vazia no modelo cliente; no modelo de referência lista o `autoincrement` das 12 chaves, as 12 chaves estrangeiras `DEFERRABLE` (das 14), as 3 chaves estrangeiras sem chave no alvo, as 20 colunas `String` sem comprimento; o comentário de tabela e de coluna é opcional (decisão do usuário de 2026-09-21), e o da coluna, quando existe, vai para o esquema Arrow e para o Delta. |
...
- **`cast`** despacha por `isinstance` para `_cast_batch`, `_cast_table` e `_cast_reader`, e as três
  ...
  ida e volta devolve o valor igual), `_refuse_time_zone_change` (`timestamp` numa coluna
  `DateTime` só quando os dois têm fuso ou nenhum tem: tirar ou pôr o fuso muda a hora gravada,
  e o mesmo 12:00 UTC saiu 12:00 do `cast` do PyArrow e 09:00 do `CAST` do DuckDB na sessão
  `America/Sao_Paulo`, leitura de 2026-09-23 em [`POC.md`](POC.md); decisão do usuário de
  2026-09-23) e `_refuse_nested_json` (`struct`, `list` e `map` numa coluna JSON). Um
  `timestamp` de outro fuso numa coluna com fuso entra no mesmo instante, em UTC. A conversão, `_converted`, leva o `arrow.uuid` ao texto canônico por `_uuid_as_text`, os
  16 bytes em `bytes.hex` nos grupos de 8, 4, 4, 4 e 12 dígitos: o cast do PyArrow converte a
  extensão pelos bytes, e `str(uuid.UUID)` levou quatro vezes mais tempo num milhão de valores
  (leitura de 2026-09-25, [`POC.md`](POC.md)). Nos demais tipos, ela é
  `column.cast(field.type, safe=True)`, que recusa escala perdida, nanossegundo não nulo e
  estouro; o inteiro numa coluna `Numeric(p, s)` passa antes por
  ...
  caracteres), `_refuse_text_above_varchar` (texto acima de 65.535 bytes numa coluna `Text`, que
  não declara `n`: o teto do `VARCHAR` do Redshift, que `sql_type` escreve no DDL),
  `_refuse_json_above_limit` (documento JSON acima dos mesmos 65.535 bytes, o maior que o `COPY`
  de Parquet leva a `SUPER`; decisão do usuário de 2026-09-23, [etapa 8](PLAN-STAGE-8.md)) e
  `_refuse_text_above_uuid` (texto acima dos 36 bytes do `VARCHAR(36)` numa coluna `Uuid`;
  decisão do usuário de 2026-09-25). O `cast` do
...
- **`check_models`** percorre `metadata.sorted_tables` e reúne, por tabela, `_column_problems`
  ...
  `True`: a regra reprova os dois numa chave inteira. O `Enum` e o `Numeric` de precisão fora
  de 1 a 38 ou de escala fora de 0 a `p`, até 37, saem de `arrow_type` (por `_decimal_type`) como
  `ContractError`, que `_column_problems` lista como as demais violações de tipo (decisões do
  usuário de 2026-09-25). `String` sem comprimento é violação por
...
| Tipo fora do contrato | `test_arrow_schema_refuses_foreign_types`, `test_arrow_schema_refuses_numeric_outside_the_decimal`, `test_arrow_schema_accepts_numeric_at_the_decimal_limits` | `Float`, `LargeBinary`, `ARRAY`, `Interval` e `Enum` levantam `ContractError` com tabela e coluna; `Numeric(39, 2)` e `Numeric(-1, 0)` também, no lugar do `ValueError` do PyArrow, `Numeric(10, 12)` e `Numeric(38, -1)`, no lugar da `Exception` do delta-rs, e `Numeric(38, 38)`, que a documentação do Redshift recusa; `Numeric(38, 2)`, `Numeric(38, 37)`, `Numeric(10, 10)` e `Numeric(1, 0)` passam no Arrow e no esquema Delta. |
...
| Cast que recusa | `test_cast_refuses_each_loss`, parametrizado | Nulo em `NOT NULL`, `double` fora da escala, `timestamp` com hora em `Date`, `timestamp` com fuso numa coluna sem fuso e o inverso, `struct` em JSON, texto acima de `String(n)` em bytes, texto acima dos 36 bytes numa coluna `Uuid`, escala perdida, nanossegundo não nulo, estouro de inteiro, lote sem coluna do contrato; cada mensagem cita a tabela e a coluna. |
| UUID em texto | `test_cast_turns_uuid_into_its_canonical_text`, parametrizado | Um `uuid.UUID` num lote, numa tabela do `from_pandas` e num leitor entra na coluna `Uuid` e numa coluna `Text` como `str(valor)`: o nulo, o UUID de bytes nulos, o de 16 bytes ASCII e um aleatório. |
...
Outra de 2026-09-23 fechou a proposta da revisão de código de 2026-09-22: o `cast` recusa com
`ContractError` o `timestamp` com fuso numa coluna `DateTime` sem fuso e o inverso, e a instrução
manda converter no cliente. Nenhuma decisão da etapa espera o usuário.
