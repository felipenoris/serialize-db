...
| `export_partition(table, value)` | O `RecordBatchReader` da partição, passado por `cast`, para `publish_partition`, num cursor próprio; ou `COPY ... TO '<uri>/<coluna de partição>=<valor>/<execution_id>.parquet' (FORMAT parquet, RETURN_STATS)` mais `register_files` para a tabela que não cabe na memória. |
