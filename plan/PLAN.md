...
- O delta-rs não lê a região de `~/.aws/config`: com `region = us-west-2` no perfil `default` e
  ...
  alvo); o secret do DuckDB leva a chave da credencial do `boto3`, e o motor DuckDB o recria na
  entrada de cada sessão quando ela troca (decisão do usuário de 2026-09-25,
  [etapa 3](PLAN-STAGE-3.md)): no alvo, o `delta_scan` falhou uma vez com a chave vencida do
  `credential_chain` com `REFRESH auto`, que só uma leitura pelo `httpfs` renovava.
