...
- **`export_partition`** começa por `UNLOAD ('<select do contrato sem a coluna de partição>') TO
  ...
  `publish_partition`. Com `columns_without_min_max` não vazio, a partição vai por esse caminho
  mesmo com `mode="register"`, e o motor registra no log, em nível `WARNING`, a tabela, a partição,
  as colunas e que a exportação passou para `rewrite`.
...
| Troca para `rewrite` | `test_nonfinite_partition_exports_by_rewrite_with_a_warning` | Com `mode="register"` e `columns_without_min_max` não vazio, a partição sai pelo destino do `rewrite` e por `publish_partition`, e o log recebe um `WARNING` com a tabela, a partição e as colunas; sem colunas, o `register` segue e o log não recebe aviso. |
