...
- O banco do esquema do projeto e o nome em três partes: se a sessão enxerga
  `datalake_rw_shared.sbx_aco_decon`, se o `CREATE`, o `INSERT` e o `SELECT` por três partes passam,
  e se o consumidor atende aos requisitos da escrita num datashare (patch 186, isolamento de
  snapshot, 64 slices).
...
para a [etapa 5](PLAN-STAGE-5.md). O `redshift.py` no ambiente alvo é o que decide as questões de
[`OPEN_QUESTIONS.md`](OPEN_QUESTIONS.md) que a suíte não alcança sem escrever: o papel IAM do
`COPY` (`RS-6`, `RS-11`), os requisitos da escrita no datashare (`RS-17`) e onde as tabelas de
execução podem nascer (`RS-9`).
