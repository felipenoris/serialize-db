...
- **`cast`** despacha por `isinstance` para `_cast_batch`, `_cast_table` e `_cast_reader`, e as três
  ...
  `timestamp` de outro fuso numa coluna com fuso entra no mesmo instante, em UTC. A conversão, `_converted`, é `column.cast(field.type, safe=True)`, que recusa escala
  ...
  não declara `n`: o teto do `VARCHAR` do Redshift, que `sql_type` escreve no DDL) e
  `_refuse_json_above_limit` (documento JSON acima dos mesmos 65.535 bytes, o maior que o `COPY`
  de Parquet leva a `SUPER`; decisão do usuário de 2026-09-23, [etapa 8](PLAN-STAGE-8.md)). O `cast` do
