...
- **`check_models`** percorre `metadata.sorted_tables` e reúne, por tabela, `_column_problems`
  ...
  `True`: a regra reprova os dois numa chave inteira. O `Enum` e o `Numeric` de precisão acima
  de 38 saem de `arrow_type` como `ContractError`, que `_column_problems` lista como as demais
  violações de tipo (decisão do usuário de 2026-09-25). `String` sem comprimento é violação por
...
| Tipo fora do contrato | `test_arrow_schema_refuses_foreign_types`, `test_arrow_schema_refuses_numeric_above_38_digits` | `Float`, `LargeBinary`, `ARRAY`, `Interval` e `Enum` levantam `ContractError` com tabela e coluna; `Numeric(39, 2)` também, no lugar do `ValueError` do PyArrow, e `Numeric(38, 2)` passa. |
