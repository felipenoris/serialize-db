...
Cada arquivo aqui é a saída de uma execução de `probes/<script>.py` num ambiente real, guardada como
saiu. A pasta `probes/output/` fica fora do git e recebe toda execução; esta guarda as que
sustentam uma afirmação de [`../POC.md`](../POC.md), que é onde a leitura é interpretada. O nome é
`<script>-<AAAA-MM-DD>[-<HHMM>].txt`, com a hora em UTC quando há mais de uma leitura no dia.
