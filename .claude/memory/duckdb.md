...
Read before code on `engine.duckdb`, `storage.duckdb_setup`, a probe that opens DuckDB, or a query over Parquet or Delta from DuckDB. Each fact ends with the `docs/` file that details it, and `tests/proof_of_concept/` holds the API details as assertions. A fact found in a session is appended here, under the heading it belongs to.
