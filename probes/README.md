...
(`short_config`), a classificação "o serviço respondeu com erro" contra "sem resposta", DNS e TCP,
a leitura do projeto por `sagemaker_studio` no primeiro interpretador que tem o pacote
(`project_snapshot`) e o mascaramento de segredos na saída. O pacote `sagemaker-studio` não entra
no projeto: ele arrasta `deltalake`, `duckdb` e `pandas` sem versão fixa, e numa instalação de
teste rebaixou o `duckdb` para 1.5.1.
...
| A pasta preparada tem o Python e as versões fixadas, e as extensões do DuckDB carregam? | `space.py`, `SP-8`, `SP-9` e `SP-10` |
