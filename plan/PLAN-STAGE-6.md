...
| `serialize-db run` | `--root`, `--environment`, `--engine`, `--partition`, `--execution-id`, `--metadata modulo:atributo` (o `MetaData` dos modelos, como em `schema` e `sql`) e `modulo:funcao` do pipeline, que recebe `run`; código de saída 0, 1 na reprovação da auditoria, 2 no conflito e no `ContractError` da abertura ou do pipeline, como o motor que não serve, a configuração do Redshift sem conexão e o `execution_id` longo demais para o prefixo do sandbox. |
...
| Partições anteriores | `test_previous_partitions_up_to_the_execution_partition` | Só valores até a partição da execução, os `n` últimos, em ordem; a tabela ausente e a tabela sem arquivos dão a lista vazia. |
