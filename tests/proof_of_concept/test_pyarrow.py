"""O PyArrow como forma canônica dos dados: esquema com metadados, cast seguro, leitores em lote e
Parquet.

Sem gravar arquivo: o esquema com nulidade, comentários e ``PARQUET:field_id`` nos metadados de
campo, ``Table.from_pylist`` (que exige dicionários), o cast seguro que recusa perda de dados, o
...
``from_batches`` que não confere os lotes contra o esquema declarado. Sob a raiz local (marcador
``local``): o ``ParquetWriter`` lote a lote com um row group por lote e o rodapé lido de volta, o
mesmo conteúdo gravado pelo DuckDB para comparar os tipos físicos, o dataset particionado ao estilo
Hive, e a leitura por lotes com ``filters`` e o pandas com tipos Arrow.
"""
    ...
    return pa.schema(
        [
            pa.field(
                ...
                pa.int64(),
                nullable=False,
                metadata={"PARQUET:field_id": "1", "comment": "Identificador"},
            ),
            pa.field(
                "data_ref", pa.timestamp("us"), nullable=False, metadata={"PARQUET:field_id": "2"}
            ),
            pa.field(
                "valor", pa.decimal128(18, 2), nullable=False, metadata={"PARQUET:field_id": "3"}
            ),
...
def test_schema_metadata_and_from_pylist() -> None:
    """Metadados de campo e de esquema são bytes; ``from_pylist`` quer dicionários, e tuplas viram
    nulos sem erro."""
    schema = contract_schema()
    assert schema.metadata == {b"serialize_db_version": b"1"}
    assert schema.field("id_operacao").metadata[b"comment"] == b"Identificador"
    assert not schema.field("id_operacao").nullable
    assert schema.field("descricao").nullable

    row = {
        "id_operacao": 1,
        "data_ref": dt.datetime(2026, 1, 1),
        "valor": decimal.Decimal("1.50"),
        "descricao": None,
    }
    ...
    assert tuples.num_rows == 1
    assert all(column.null_count == 1 for column in tuples.columns)

    # O Arrow não valida a nulidade declarada: nulo num campo nullable=False entra; a biblioteca
    # confere null_count.
    required = pa.schema([pa.field("id_operacao", pa.int64(), nullable=False)])
    violated = pa.Table.from_pylist([{"id_operacao": None}], schema=required)
    assert violated.column("id_operacao").null_count == 1


def test_safe_cast_refuses_data_loss() -> None:
    """O cast seguro (padrão) recusa perda de precisão, estouro e perda de resolução; comprimento de
    texto é conferido à parte.

    ``double`` para ``decimal`` arredonda o valor binário sem erro, e ``timestamp`` para ``date``
    descarta a hora; ``Table.cast`` recusa nulo em campo não anulável e nomes fora de ordem, e o
    inteiro chega a ``decimal128(18, 2)`` por ``(21, 2)``; uma coluna de ``dict`` vira ``struct``, e
    um ``dict`` não entra em ``string``.
    """
    # decimal128(20, 4) com quatro casas não cabe em decimal128(18, 2); safe=False corta as casas.
    ...
    # Um int64 acima do int32 é estouro.
    with pytest.raises(pa.ArrowInvalid):
    ...
    microseconds = pa.array([1000], pa.timestamp("ns")).cast(pa.timestamp("us"))
    assert microseconds[0].as_py() == dt.datetime(1970, 1, 1, 0, 0, 0, 1)

    # O tipo string não tem comprimento, e o String(200) do contrato é conferido à parte, em bytes,
    # a medida do VARCHAR(n) do Redshift: binary_length conta bytes, utf8_length conta caracteres.
    texts = pa.array(["ok", "ç" * 101])
    assert pc.max(pc.binary_length(texts)).as_py() == 202
    assert pc.max(pc.utf8_length(texts)).as_py() == 101

    # double para decimal arredonda o valor binário exato (2,675 é 2,67499...) sem acusar a perda,
    # mesmo com safe=True; o valor representável na escala é o que pc.round devolve igual, e
    # pc.round difere do cast em 2,675.
    floats = pa.array([1.236, 2.675, 1234.56, 0.29])
    cast_to_decimal = floats.cast(pa.decimal128(18, 2))
    assert [str(value) for value in cast_to_decimal] == ["1.24", "2.67", "1234.56", "0.29"]
    rounded = pc.round(floats, 2)
    assert pc.equal(rounded, floats).to_pylist() == [False, False, True, True]
    assert [str(value) for value in rounded] == ["1.24", "2.68", "1234.56", "0.29"]

    # timestamp para date descarta a hora sem erro; a ida e volta acusa.
    stamps = pa.array(
        [dt.datetime(2026, 8, 1, 12, 30), dt.datetime(2026, 8, 2)], pa.timestamp("us")
    )
    round_trip = stamps.cast(pa.date32()).cast(pa.timestamp("us"))
    assert pc.equal(round_trip, stamps).to_pylist() == [False, True]

    # Table.cast recusa nulo em campo não anulável e exige os mesmos nomes na mesma ordem; inteiro
    # em decimal128(18, 2) pede precisão 21 e passa por (21, 2).
    with_null = pa.table({"n": pa.array([1, None], pa.int64())})
    with pytest.raises(ValueError, match="non-nullable"):
        with_null.cast(pa.schema([pa.field("n", pa.int64(), nullable=False)]))
    ...
    through_21 = pa.array([1]).cast(pa.decimal128(21, 2)).cast(pa.decimal128(18, 2))
    assert through_21.to_pylist() == [decimal.Decimal("1.00")]

    # Uma coluna de dicts vira struct com a união das chaves, e um dict não entra em string: JSON
    # chega serializado.
    frame = pd.DataFrame({"atributos": [{"k": 1}, {"k": 2, "x": "y"}]})
    documents = pa.Table.from_pandas(frame, preserve_index=False)
...
def test_arrow_table_round_trips_through_pandas_without_copy() -> None:
    ...
    assert [str(dtype) for dtype in frame.dtypes] == [
        "int64[pyarrow]",
        "date32[day][pyarrow]",
        "decimal128(18, 2)[pyarrow]",
        "string[pyarrow]",
    ]
    # __arrow_array__ devolve o ChunkedArray que a coluna ArrowDtype guarda, o próprio objeto.
    assert addresses(frame["valor"].array.__arrow_array__()) == addresses(table.column("valor"))

    # A volta também não copia; todo campo volta anulável, e o cast seguro devolve o esquema do
    # contrato.
    back = pa.Table.from_pandas(frame, preserve_index=False)
    assert back.schema.types == schema.types
    assert all(field.nullable for field in back.schema)
    ...
    with_null = pa.table({"n": pa.array([1, None], pa.int64())}).to_pandas()
    assert str(with_null["n"].dtype) == "float64"
...
def test_record_batch_reader_is_consumed_once() -> None:
    """O ``RecordBatchReader`` entrega cada lote uma vez, de uma lista ou de um gerador, sem
    materializar o todo."""
    batches = []
    for i in range(3):
        ids = pa.array(range(i * 10, i * 10 + 10), pa.int64())
        batches.append(pa.RecordBatch.from_pydict({"id": ids}))
...
def test_record_batch_cast_and_conversions_share_buffers() -> None:
    """``RecordBatch.cast`` recusa o que ``Table.cast`` recusa; ``to_batches``, ``from_batches`` e o
    ciclo do lote com o pandas por ``ArrowDtype`` compartilham os buffers."""
    schema = pa.schema(
        [pa.field("n", pa.int64(), nullable=False), pa.field("valor", pa.decimal128(18, 2))]
    )
    batch = pa.RecordBatch.from_pydict(
        {
            "n": pa.array([1, None], pa.int64()),
            "valor": pa.array([decimal.Decimal("1.5000"), None], pa.decimal128(20, 4)),
        }
    )
    with pytest.raises(ValueError, match="non-nullable"):
        batch.cast(schema)
    out_of_order = pa.RecordBatch.from_pydict({"b": [1], "a": [2]})
    with pytest.raises(ValueError, match="field names"):
        out_of_order.cast(pa.schema([("a", pa.int64()), ("b", pa.int64())]))
    wide_valor = pa.array([decimal.Decimal("1.2345")], pa.decimal128(20, 4))
    wide = pa.RecordBatch.from_pydict({"valor": wide_valor})
    ...
    assert addresses(rebuilt.column("valor")) == addresses(table.column("valor"))
    ...
    frame_valor = frame["valor"].array.__arrow_array__()
    assert addresses(frame_valor) == addresses(batches[0].column("valor"))
...
def test_record_batch_reader_from_batches_trusts_the_batches() -> None:
    """``from_batches`` não confere cada lote contra o esquema declarado: ``read_next_batch``
    devolve o lote como veio, e só ``read_all`` acusa.
    ...
    def generate() -> Iterator[pa.RecordBatch]:
        try:
            for k in range(3):
                ...
                columns = {"id": pa.array([k], pa.int64()), "valor": pa.array([0.0])}
...
def test_parquet_writer_row_groups_and_footer(local_location: LocalLocation) -> None:
    ...
    for index, field in enumerate(sample.schema):
        field_id = {"PARQUET:field_id": str(index + 1)}
        nullable = field.name == "descricao"
        fields.append(pa.field(field.name, field.type, nullable=nullable, metadata=field_id))
    ...
    assert metadata.num_rows == 30_000
    ...
    assert valor.physical_type == "FIXED_LEN_BYTE_ARRAY"
    assert valor.statistics.has_min_max
    valor_statistics = f"min={valor.statistics.min} max={valor.statistics.max}"
    record("pyarrow.parquet_valor_statistics_row_group_0", valor_statistics)
    ...
    duck_schema = str(pq.ParquetFile(duck_path).schema)
    assert re.search(r"optional int64( field_id=-?\d+)? id_operacao", duck_schema)
...
def test_parquet_streaming_read_filters_and_pandas(local_location: LocalLocation) -> None:
    ...
    february = pq.read_table(
        path, filters=[("mes", "=", MONTHS[1])], columns=["id_operacao", "valor"]
    )
    assert february.num_rows == 1000
    assert february.column_names == ["id_operacao", "valor"]
